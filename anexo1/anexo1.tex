...
Esta regla no tiene ninún inconveniente en su ejecución cuando no existe un archivo llamado \textit{clean} dentro de las rutas de búsquda del cosntructor, dado que no tiene nigún pre-requisito esta regla siempre se ejecuta al ser invocada. Pero si en la ruta del constructor existe un archivo de nombre \textit{clean} el constructor determina que el \textit{target} de dicha regla se encuentra al día y no se debe ejecutar sus comandos de construcción. Para evitar esto se declara la regla de la siguiente manera:
