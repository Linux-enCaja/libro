...
Para el desarrollo de proyectos de software libre se utiliza la herramienta de construcción \textit{make}. Esta herramienta desarrolla de manera automática los pasos necesarios para la generación de un archivo de implmentación de un proyecto. La utilidad \textit{make} comunmente llamada constructor, hace parte de las utlidades entregadas por la comunidad de software libre (GNU). Donde, la función principal de \textit{make} es controlar la generación de ejecutables y otros archivos auxiliares a partir de un código fuente. Para esto \textit{make} utiliza un archivo de nombre \textbf{Makefile}, donde se encuentra la especificación del proyecto. Dicha especificación se realiza mediante la utilización de elementos del constructor como reglas, \textit{targets} y comandos de ejecución. Dada la flexibilidad de la herramienta \textit{make}, se utiliza para construir cualquier tipo de proyecto que requiera muchas etapas de compilación, síntesis o simulación.

\begin{figure}[h]
  \begin{center} \includegraphics[scale=.75]{./anexo1/images/toolchain/make} \end{center}
  \caption{Herramienta \textit{make} y Makefile}\label{make}
\end{figure}


	\subsection{Target, prerequisitos y reglas asociadas}

El \textit{make} toma las reglas del archivo \textit{Makefile} para determinar qué archivo se debe debe reconstruir para obtener el archivo binario de la aplicación. Esta desición se toma en base a las marcas de tiempo de los archivos fuentes, y el de salida (\textit{target}). Las marcas de tiempo son campos dentro del archivo que indican la fecha de creación y la fecha de la última modificación del archivo. Con base en esto, sí los archivos fuentes que son prerequisitos de una regla tiene una marca de tiempo más reciente, el constructor determina que este archivo se debe reconstruir.\\

Las reglas del constructor se componen de un \textit{target} o archivo de salida (generalmente un archivo de salida por regla) y una serie de pre-requisitos para cada regla. Los pre-requisitos de la regla se deben encontrar en las rutas de búsqueda del constructor. Sí algún pre-rerquisito no existe el construcctor buscará una regla asociada para su construción y procederá a generar dicho archivo. Dado el caso que no exista algún archivo pre-requisito ni una regla asociada a este, el constructor falla con un mensaje de error de falta de pre-requisitos. A continuación se muestra un ejemplo de la sintaxis de una regla para compilar \textit{foo.c} en \textit{foo.o}
...
\subsubsection{Reglas explicitas:} Las reglas explícitas son las que constructor toma cuando un \textit{target} no se encuentra actualizado. El anterior ejemplo muestra como una regla explicita asociada al \textit{target1.o}. En algunas ocaciones dos o más \textit{targets} tienen los mismos prerequisitos para estos casos se pueden definir todos los \textit{targets} en una lista y asociarlos a una sola regla:

\begin{lstlisting}
target1.o target2.o:	source1.c header1.h
\end{lstlisting}

\subsubsection{Reglas basadas en comodines \textit{Wildcards}:}
...
\subsubsection{Reglas de ejecución imperativa (.\textit{PHONY}):}
...
\caption{Variables automáticas}
	...
	$@$ 		& Denota el nombre del \textit{target} de la regla.\\
	$%$		& Denota el nombre del archivo dentro de una lista.\\
	$<$$	& Es el nombre del primer prerequisito de la regla.\\
	$?$		& El nombre de todos los prerequisitos de la regla y que tienen una marca de tiempo mayor al \textit{target} en una lista separada poro espacios\\
	\textit{\^}	& Es la lista de todos los prerequisitos de la regla separada por espacios. Los nombres repetidos son automáticamente eliminados para evitar ejecuciones reentrantes en el construcctor.\\
	$+$		& Es la lista de todos los prerequisitos de la regla separados por espacios, esta lista incluye nombres repetidos. \\
	$*$		& Es el nombre del archivo de salida sin su extensión de salida.
	\end{tabular}
\end{table}
